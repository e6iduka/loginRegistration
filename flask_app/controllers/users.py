...
def register():
    ...
    if User.get_user_by_email(request.form):
        flash("THIS EMAIL EXIST", 'emailRegister')
    ...
    if 'password' not in request.form:
        flash("Password is required", 'registrationFailed')
        return redirect(request.referrer)
...
def login():
    ...
    if not User.get_user_by_email(request.form):
        flash('This email dosent appear to be in our system!Try another one', 'emailLogin')
        return redirect(request.referrer)
